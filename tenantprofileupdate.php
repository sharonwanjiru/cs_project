            ...
            if($fileExtension == 'jpg' || $fileExtension == 'png' || $fileExtension == 'jpeg'){
                ...
                if($fileSize < 5000000){
                    ...
                    if($uploaded){
                        ...
                        $sql = "UPDATE tenants SET username = '$userNewName', email ='$userNewEmail', image='$imageName' WHERE username = '$loggedInUser'";

                        $results = mysqli_query($conn,$sql);

                        header('Location: tenantprofile.php?success=userUpdated');
                    exit;
                    }


                }else{
                    header('Location: tenantprofile.php?error=invalidFileSize');
                    exit;
                }
                exit;
            }else{
                header('Location: tenantprofile.php?error=invalidFileType');
                exit;
            }

        }else{
            header('Location: tenantprofile.php?error=emptyNameAndEmail');
            exit;
        }
